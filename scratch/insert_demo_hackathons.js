...
const organizers = [
  ...
  "HackMIT Global", "Stanford d.school", "NITI Aayog", "Digital India"
];
...
async function insertHackathons() {
  try {
    const response = await fetch(`${supabaseUrl}/rest/v1/hackathons`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      console.log('Successfully inserted 50 hackathons into Supabase!');
    } else {
      const errorText = await response.text();
      console.error('Failed to insert hackathons:', response.status, errorText);
    }
  } catch (err) {
