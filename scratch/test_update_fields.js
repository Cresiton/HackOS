...
async function test() {
  ...
  // 3. Try updating only title
  console.log("3. Updating only title...");
  const res1 = await supabase
    .from("hackathons")
    .update({ title: "Only Title Updated" })
    .eq("id", hackathon.id)
    .select();
  console.log("Result 1 (title only):", res1);

  // 4. Try updating only description (with metadata unchanged)
  console.log("4. Updating only description (same metadata)...");
  const res2 = await supabase
    .from("hackathons")
    .update({ description: serializedDescription })
    .eq("id", hackathon.id)
    .select();
  console.log("Result 2 (description only):", res2);

  // 5. Try updating description to a plain string without metadata
  console.log("5. Updating description to plain string...");
  const res3 = await supabase
    .from("hackathons")
    .update({ description: "Plain description without metadata" })
    .eq("id", hackathon.id)
    .select();
  console.log("Result 3 (plain description):", res3);
}
